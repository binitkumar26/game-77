       ...
       if(validateToken($access_token,$user_id)=='TRUE'){
         	...
         	// Fetch Type blog
				...
				if($_POST['user_id']){
					...
					$where =" WHERE user_id='".$user_id."' and wallet_type='commision'";
				}

				else{

					$where =" WHERE wallet_type='commision'";
				}


		        // Fetch All Blog details
				$select= "SELECT * from tbl_wallet".$where;
				...
				while($data=mysqli_fetch_assoc($query)){
					...
					$result[] = array(
									...
									"status" => $data['status'],
									"datetime" =>date("Y-m-d \n H:i:sa",strtotime($data['datetime']))
